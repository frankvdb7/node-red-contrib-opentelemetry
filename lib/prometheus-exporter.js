...
function startHttpInExporter (port, endpoint, instrumentName, serviceName = 'Node-RED') {
  return new Promise((resolve, reject) => {
    ...
    exporter = new Exporter({
      ...
      const ExplicitBucketHistogramAggregationImpl = dependencies.ExplicitBucketHistogramAggregation
      const ResourceImpl = dependencies.Resource
      const meterProvider = new MeterProviderImpl({
        views: [
          new ViewImpl({
            aggregation: new ExplicitBucketHistogramAggregationImpl([0, 25, 50, 75, 100, 250, 500, 1000, 2000]),
            instrumentName,
          }),
        ],
        readers: [exporter],
...
module.exports = {
  ...
  __test__: {
    ...
    getState: () => ({ exporter, requestDurationHistogram }),
    setRequestDurationHistogram: (histogram) => { requestDurationHistogram = histogram },
