...
module.exports = function (RED) {
	...
	function OpenTelemetryNode(config) {
		...
		const ignoredTypesList = splitCsv(ignoredTypes);
		const propagateHeadersTypesList = splitCsv(propagateHeadersTypes);
		_isLogging = Boolean(isLogging);
		_rootPrefix = rootPrefix;
		_timeout = normalizeTimeoutMs(timeout);
		_attributeMappings = sanitizeAttributeMappings(attributeMappings);
		...
		RED.hooks.add("onSend.otel", (events) => {
			...
			events.forEach((event) => {
				logEvent(node, "1.onSend", event);
				createSpan(
					tracer,
					event.msg,
					event.source.node,
					node,
					ignoredTypesList.includes(event.source.node.type),
				);
			});
		});
		...
		RED.hooks.add("postDeliver.otel", (sendEvent) => {
			...
			const span = createSpan(
				...
				node,
				ignoredTypesList.includes(sendEvent.destination.node.type),
			);
...
module.exports.__test__ = {
	...
	setAttributeMappings: (mappings) => {
		_attributeMappings = sanitizeAttributeMappings(mappings);
	},
