...
module.exports = function (RED) {
	...
	function OpenTelemetryNode(config) {
		...
		const propagateHeadersTypesList = propagateHeadersTypes
			.split(",")
			.map((key) => key.trim());
		...
		RED.hooks.add("onSend.otel", (events) => {
			...
			createSpan(
				tracer,
				events[0].msg,
				events[0].source.node,
				node,
				ignoredTypesList.includes(events[0].source.node.type),
			);
		});
		...
		RED.hooks.add("preDeliver.otel", (sendEvent) => {
			if (propagateHeadersTypesList.includes(sendEvent.source.node.type)) {
				...
				propagation.fields().forEach((field) => {
